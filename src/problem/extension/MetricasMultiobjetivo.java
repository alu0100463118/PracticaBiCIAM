...
// % de soluciones q no son miembros del frente de pareto verdadero
	public double TasaError(List<State> solutionsFPcurrent, List<State> solutionsFPtrue) throws BadDepthException, IOException{
		float tasaError = 0;
...
// % Indica  qun  tan  lejos  estan  los  elementos  del frente  de  Pareto  actual  respecto  al  frente  de  Pareto  verdadero	
	public double DistanciaGeneracional(List<State> solutionsFPcurrent, List<State> solutionsFPtrue) throws BadDepthException, IOException{
		float min = 1000;
		float distancia = 0;
		float distanciaGeneracional = 0;
		for (int i = 0; i < solutionsFPcurrent.size();i++) {
			State solutionVO = solutionsFPcurrent.get(i);
			//Calculando la distancia euclideana entre solutionVO y el miembro mas cercano del frente de Pareto verdadero
			min = 1000;
			for (int j = 0; j < solutionsFPtrue.size();j++) { 
				for (int j2 = 0; j2 < solutionVO.getEvaluation().size(); j2++) {
					...
					distancia += (solutionVO.getEvaluation().get(j2) - solutionFPV.getEvaluation().get(j2))*  
							(solutionVO.getEvaluation().get(j2) - solutionFPV.getEvaluation().get(j2)); //ceros si el argumento es el cero, 1.0 si el argumento es mayor que el cero, -1.0 si el argumento esta menos del cero
				}
				if(distancia < min){
					min = distancia;
				}
	...
	public double Dispersion(ArrayList<State> solutions) throws BadDepthException, IOException{
		//Soluciones obtenidas con la ejecucian del algoritmo X
